...
export async function GET(req: Request) {
  ...
  if (!code || !state || !expected || state !== expected) {
    return NextResponse.redirect(new URL("/mail?error=oauth_state", req.url));
  }
  ...
  try {
    ...
    if (!tokens.refresh_token) {
      ...
      return NextResponse.redirect(new URL("/mail?error=oauth_norefresh", req.url));
    }
    ...
    return NextResponse.redirect(new URL("/mail?connected=gmail", req.url));
  } catch (e) {
    console.error("[gmail oauth] callback failed", e);
    return NextResponse.redirect(new URL("/mail?error=oauth_exchange", req.url));
  }
}
