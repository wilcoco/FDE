...
## 0. MVP 선언 (창업자 지시, 2026-08-16 — 최우선 원칙)

**본질 세 동작만 남긴다: ① 지시하기 → ② 답변 받기 → ③ 수행됨 처리.**
- **일의 전후 원칙**: 꼭지 분해는 반드시 **발송 전에** 일어나고, 그 꼭지가 **메일 본문과 직답 페이지에 그대로 실린다** — 상대는 무엇에 답해야 하는지 보면서 답한다. 발송 후 분해는 금지 (상대가 본 적 없는 구조에 답이 맞을 리 없다).
- **입구 단일화**: "지시하기"(/capture)가 유일한 생성 입구. 받는 사람 이메일을 적으면 메일 발송(+꼭지+직답 버튼)까지 한 번에, 비우면 사내 지시. /mail은 연결·답장 확인용 보조 화면.
- **메뉴 숨김**: 전략 통일성 · 목표(OKR·KPI) · 분석 · 조직도는 nav에서 제거 (라우트는 생존, layout.tsx에 복원 방법 주석). 새 기능 제안이 이 세 동작 밖이면 기본 답은 "나중에".
...
- **제품 3층 구조 (창업자 재정의, 2026-08-13)**: ① 핵심 가치 = AI가 사용자의 메일함에서 say-do를 캐내 장부를 자동 생성 (지시를 "작성"시키지 않는다 — 입력 병목의 해법) ② 로컬(브라우저) 저장 = 핵심이 아니라 수단: CASA 회피 + "원본은 기기 밖으로 안 나감" 신뢰 문장 ③ 직답 + 팀 장부 = 수익화 루프 (①만으론 개인 도구). 한 줄: "이미 보낸 메일에서 AI가 약속 장부를 만들어드립니다 — 원본은 당신 기기 밖으로 나가지 않습니다." 외부 리뷰의 "자동 추출이 진짜 제품" 결론과 독립적으로 수렴.
...
8. (후속, 2026-08-17 발송 재설계에서 파생) ① `/mail`의 "메일로 맡기기" 카드는 이제 /capture 빠른 보내기와 중복되는 두 번째 입구 — composeAndSend의 발송 실패 echo를 /capture로 옮긴 뒤 제거 ② 이름↔이메일 원장: 직답의 "성함" 입력을 counterparty 이메일과 묶어 축적하면 "김부장" 같은 이름 매칭 추천 가능 ③ IMAP/POP3 보낸함에서도 수신자 수확(서버 경로).

### 7-가. Gmail 메일함 접근의 대안 지도 (2026-08-12 창업자 문답으로 확정)

응답률 데이터가 "직답만으로 부족"을 말할 때 꺼내는 카드, 서열 순:
1. **직답 (현재)** — 감사 0, 마찰 0, 구조화 공짜. 이미 프로덕션 작동.
2. **브라우저-로컬 하이브리드 (창업자 설계, 2026-08-12) — ✅ 구현됨 (2026-08-13, `b08d91f`)**: `src/lib/gmail-local.ts`(순수 파싱) + `src/components/LocalGmail.tsx`(GIS 토큰, 브라우저 직접 조회) + `src/app/actions/local-mail.ts`(선택 결과만 수신). /mail의 Gmail 연결에 표시. 선행조건: 메일 OAuth 웹 클라이언트에 앱 origin을 "승인된 JavaScript 원본"으로 등록. 답장 감지도 [기록] 버튼 클릭으로 확정 = user-configured 논리 유지. — 원본 메일·구글 토큰은 **브라우저에만**(서버는 중계·보관·접근능력 전무), 서버엔 사용자가 "지시로 등록"을 눌러 올린 say-do 장부만. 우리의 건별 옵트인 UX("지시로 등록"/"+본문")가 구글 면제 조항 문구 "user-configured transmissions"와 정합 → **CASA(유료 연례평가) 면제 가능성**. 실행 조건: ①"답장 도착" 자동 핑의 면제 해당 여부 구글 사전 질의(회색지대) ②앱 검증(무료·수주)은 여전히 필요, 100명 캡은 검증 통과까지 유지 ③브라우저 토큰 ~1h 수명 = pull 모델과는 정합, 백그라운드 감시는 불가(그건 직답 담당) ④"한 글자도 못 읽습니다" 영업 멘트 반납. 팀 대시보드·검토 게이트는 서버 장부라 그대로 유지 — "전부 로컬이면 그룹웨어 깨짐"이라던 이전 기각은 오류였음(메일 저장과 장부 저장의 혼동).
3. **CASA 구매** — gmail.readonly 복원 + 연례 보안평가(수주+실비+연 갱신). 웹 아키텍처 유지, 매출이 비용을 정당화할 때. 코드는 하루면 복원(제거된 metadata 분기가 청사진). ②′가 질의에서 막히면 이 카드.
4. **완전 로컬 데스크톱 컴패니언** — Thunderbird 모델(모든 데이터·토큰 기기 안). 설치 마찰 + 새 플랫폼 비용. 최후의 카드.
+ 레거시/비Gmail은 애초에 IMAP 앱비밀번호로 전부 열려 있음 (길 A) — 서버는 지금도 pull 중계만 하고 보관하지 않음(mail-fetch.ts 설계 원칙).
