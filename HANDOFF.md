...
2. **CASA 구매** — gmail.readonly 복원 + 연례 보안평가(수주+실비+연 갱신). 웹 아키텍처 유지, 매출이 비용을 정당화할 때. 코드는 하루면 복원(제거된 metadata 분기가 청사진).
3. **로컬 클라이언트 면제** — Google 정책상 실존: 메일 데이터가 기기 밖으로 안 나가는 로컬 앱은 CASA 면제 (Thunderbird 모델). 단 토큰도 기기 안, 서버는 "접근 능력"조차 금지, 파생 신호(답장 도착 핑) 전송은 회색지대(사전 질의 필요). 팀 공유·서버 장부와 정면 충돌 → 데스크톱 컴패니언(로컬 답장 감지기)으로만 유효. 최후의 카드.
+ 레거시/비Gmail은 애초에 IMAP 앱비밀번호로 전부 열려 있음 (길 A).
